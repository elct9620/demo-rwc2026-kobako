...
against, which is `gpt-5.6-luna`. The writer needs tools and reasoning at
once, and newer models only carry that pair on OpenAI's Responses API — the
protocol `ruby_llm` uses for OpenAI by default.
